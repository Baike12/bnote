...
- **插入链接（`Cmd+K`）**：在光标处弹出笔记补全——空输入列出最近打开（与快速跳转同一套排序：文件名命中优先，文件夹名可搜），输入即模糊过滤，回车插入 `[[链接]]`（有选区时选区文字当别名）；**本行已经有链接时同一个键改为跳转到那篇笔记**（一行只放一个链接）；重名笔记自动写成仓库相对路径，代码块/行内代码里的 `[[…]]` 当作示例文本不参与
- **回退链接跳转（`Cmd+[`）**：沿着链接跳转链往回走，跳了几层就能退几层；已经是最早一层、或中间用侧栏/快速跳转等别的方式打开过文件（链已断）时无效
